...
class TaskNotificationReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        val identifier = intent.getStringExtra(EXTRA_TASK_IDENTIFIER).orEmpty()
        ...
        val notificationId = abs((identifier + (deadlineIso ?: deadlineMillis)).hashCode())
        ...
        val formattedDateTime = formatDateTimeForDisplay(context, deadlineIso, deadlineMillis)
        val promptText = context.getString(R.string.task_notification_prompt)
        val quadroText = quadroNome?.takeIf { it.isNotBlank() }
        ...
        val historyMessage = listOfNotNull(promptText, formattedDateTime, quadroText.takeIf { it.isNotBlank() })
            .joinToString("\n")
    ...
    private fun formatDateTimeForDisplay(
        context: Context,
        dateTimeString: String?,
        fallbackMillis: Long?
    ): String {
        val zoned = TaskNotificationScheduler.parseDateTime(dateTimeString)
            ?: fallbackMillis?.let { Instant.ofEpochMilli(it).atZone(java.time.ZoneId.systemDefault()) }
