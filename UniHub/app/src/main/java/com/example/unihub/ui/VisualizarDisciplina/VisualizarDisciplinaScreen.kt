...
fun VisualizarDisciplinaScreen(
    disciplinaId: Long?, // Corrigido para Long? pois no modelo Disciplina o id é Long
    onVoltar: () -> Unit,
    onNavigateToEdit: (Long) -> Unit // Ação para navegar para edição usando Long
) {
    val context = LocalContext.current

    // Simulação de busca de dados - substitua pela sua implementação real
    val disciplina = remember(disciplinaId) {
        Disciplina(
            disciplinaId = disciplinaId ?: 0L,
            codigo = "DS123", // código da disciplina (String)
    ...
    val opcoes = listOf(
        OpcaoDisciplina("Informações da disciplina", Icons.Outlined.Info, Color(0xFFD7EFF5)) {
            onNavigateToEdit(disciplina.disciplinaId)
        },
